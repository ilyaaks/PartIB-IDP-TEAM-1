...
class LineFollowerRobot:
    ...
    BASE_SPEED = 65           # Default speed for straight line following
    ...
    def _init_sensors(self):
        ...
        self.red_led = Pin(10, Pin.OUT)  # Control pin for colour sensor's red LED
        self.red_led.value(1)  # Turn off red LED initially
    ...
    def _init_state(self):
        ...
        self.current_speed = self.BASE_SPEED  # Track current target speed for line following
    ...
    def motor_go_straight(self, speed_left, speed_right, delay=0):
        ...
        Returns:
            None
        """
        # Update current_speed if both wheels are moving at the same speed
    ...
    def motor_turn(self, speed_left, speed_right):
        ...
        if speed_left >= 0 and speed_right >= 0:
            ...
            self.motor_left.Forward(speed_left)
            self.motor_right.Reverse(speed_right)
            # Don't update current_speed during turns (asymmetric wheel speeds)
        elif speed_left <= 0 and speed_right >= 0:
    ...
    def line_follower(self, p):
        ...
        if sensor_mid_right == 1 and sensor_mid_left == 0:
            ...
            if self.sensor_left_prev == 1 and self.sensor_right_prev == 0:
                ...
                # On track - maintain current speed
                self.left_wheel_speed = self.current_speed
                self.right_wheel_speed = self.current_speed
    ...
    def _before_pick_box(self, current_position: str):
        ...
        if current_position[0:2] == "B0" or current_position[0:2] == "A1":
            ...
            self.motors_off()
            #Check if the distance to the box is correct 
    ...
    def _approach_box(self):
        ...
        self.motor_go_straight(self.BASE_SPEED, self.BASE_SPEED)
        start_time = ticks_ms()
        # Approach until within 62mm or timeout after 5 seconds
        while (self._calculate_distance() > 62):
            #TODO: Work out how long to delya
            if ticks_diff(ticks_ms(), start_time) > 1500:
    ...
    def _place_box(self):
        ...
        self.linear_actuator.off()
        self.direction_flag = "reverse"
        self.motor_go_straight(self.BASE_SPEED, self.BASE_SPEED, delay=0.6)
        self.motors_off()
        self.linear_actuator.set("retract", 90)
        sleep(4.9)
        self.linear_actuator.off()
    ...
    def run(self):
        ...
        self.direction_flag = "forward"
        self.motor_go_straight(self.BASE_SPEED, self.BASE_SPEED)
        self._path_algorithm(current_position,next_position)
        self._place_box()    
        #Now the box is in whatever bay:
        self.motors_off()
        # Re-initialize VL53L0X for the next box detection
        self._reinit_vl53l0()
        sleep(0.1)
        self.direction_flag = "forward"
        self.motor_go_straight(self.BASE_SPEED,self.BASE_SPEED)
        current_position, next_position = self.pick_box(start=next_position,destination="A01")
        #Now have to go upstairs, coz the box has been taken

        

        # self._do_pick_box()
        # self._after_pick_box("A01")
